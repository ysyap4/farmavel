                ...
                <div class="col-lg-8">
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                            <form method="POST" class="form-horizontal" action="{{ URL::route ('manage_user_create_process')}}" novalidate role="form" enctype="multipart/form-data">
    ...
    <script>
        ...
        function readURL(input) {
            if (input.files && input.files[0]) {
                ...
                var width = (input.files[0]).width();    // Current image width
                var height = (input.files[0]).height();  // Current image height
            
                // Check if the current width is larger than the max
                if(width > maxWidth){
                    ratio = maxWidth / width;   // get ratio for scaling image
                    (input.files[0]).css("width", maxWidth); // Set new width
                    (input.files[0]).css("height", height * ratio);  // Scale height based on ratio
                ...
                if(height > maxHeight){
                    ...
                    $(input.files[0]).css("height", maxHeight);   // Set new height
                    $(input.files[0]).css("width", width * ratio);    // Scale width based on ratio
